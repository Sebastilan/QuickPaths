...
- **技术栈**：C# WPF + csc.exe 编译（零外部依赖，.NET Framework 4.x 自带）
- **路径**：`C:\Users\ligon\CCA\QuickPaths`
- **当前阶段**：v2.0 C# 重写完成。下一步见 TODO.md「未来可选」

## 文件结构

| 文件 | 用途 |
|------|------|
| `QuickPaths.cs` | 主程序源码（WPF 悬浮窗 + 全部逻辑 + install/uninstall） |
...
1. **WPF 逻辑像素 vs WinForms 物理像素**：定位用 `SystemParameters.WorkArea`，不用 `Screen.WorkingArea`
2. **多显示器坐标可为负**：副屏在上方/左方时坐标负值，验证用 `VirtualScreen*` 系列属性
3. **FolderBrowserDialog 导致面板收起**：对话框夺焦触发 Deactivated，用 `dialogOpen` 标记保护
4. **剪贴板可能被占用**：`Clipboard.SetText` 需 try-catch，其他程序锁住时会抛异常
5. **CSDN 外链图片转存失败**：jsDelivr CDN / GitHub raw URL 都会被 CSDN 拦截。必须手动上传到 CSDN 图床
6. **Dispatcher.Invoke 死锁**：非 UI 线程回调中用 `Dispatcher.Invoke` 可能死锁。一律用 `Dispatcher.BeginInvoke`（异步）
7. **显示器变更防抖**：拔/插显示器会连续触发多次 `DisplaySettingsChanged`，用 DispatcherTimer 1500ms 防抖
8. **删文件≠删引用**：废弃外部资源时必须同步清理所有引用方（计划任务、注册表、Startup 快捷方式等）
9. **csc.exe 找不到 WPF 程序集**：需要 `/lib:` 指向 `Framework64\v4.0.30319\WPF` 目录
10. **csc.exe 版本仅支持 C# 5**：不能用 `$""` 字符串插值、`?.` 空条件运算符、`=>` 表达式体成员
